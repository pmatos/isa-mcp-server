...
class TestCrossISAImports:
    ...
    async def test_arm_and_intel_import(self, sample_arm_data_dir, sample_xed_data_dir):
        ...
        try:
            ...
            # Count should be > 0 but we can't match exact import count
            # due to search limits
            assert len(intel_instructions) > 0 or intel_count > 0
            assert len(arm_instructions) > 0 or arm_count > 0

            # Verify architecture existence (simpler check since fixture
            # has limited data)
    ...
    @pytest.mark.asyncio
    async def test_import_order_independence(
        self, sample_arm_data_dir, sample_xed_data_dir
    ):
        ...
        try:
            ...
            intel_result1 = await intel_importer1.import_from_source(
                sample_xed_data_dir
            )
            ...
            intel_result2 = await intel_importer2.import_from_source(
                sample_xed_data_dir
            )
            ...
            assert (
                arm_result1["stats"]["instructions_inserted"]
                == arm_result2["stats"]["instructions_inserted"]
            )
            assert (
                intel_result1["stats"]["instructions_inserted"]
                == intel_result2["stats"]["instructions_inserted"]
            )
    ...
    async def test_architecture_separation(
        ...
        try:
            ...
            intel_as_arm = db.get_instruction(
                "MOVSX", "aarch64"
            )  # MOVSX is x86-specific
            ...
            for reg in arm_regs[:5]:  # Check first 5
                ...
                assert reg.register_name.startswith(
                    ("X", "W", "V", "S", "D", "H", "B")
                ) or reg.register_name in ["SP", "PC", "XZR", "WZR", "PSTATE"]
    ...
    async def test_metadata_isolation(self, sample_arm_data_dir, sample_xed_data_dir):
        ...
        try:
            ...
            assert arm_result.get("metadata_imported", False)
            assert intel_result.get("metadata_imported", False)
            ...
            # Check for no overlap in register names (some common names
            # like SP exist in both)
