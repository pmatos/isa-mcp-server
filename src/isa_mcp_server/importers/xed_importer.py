...
from .xed_parser import XEDParser, XEDInstruction
from ..isa_database import InstructionRecord, OperandRecord, EncodingRecord
...
class XEDImporter(ISAImporter):
    ...
    async def parse_sources(self, source_dir: Path) -> Iterator[InstructionRecord]:
        ...
        extension_dirs = [
            "avx", "avx512f", "avx512cd", "avx512ifma", "avx512vbmi",
            "avx512-bf16", "avx512-fp16", "avx512-skx", "avx-vnni",
            "hsw", "hswavx", "hswbmi", "bdw", "skl", "knl", "knm",
            "cet", "sha", "gfni-vaes-vpcl", "clwb", "clflushopt",
            "movdir", "enqcmd", "serialize", "tsxldtrk", "amx-spr",
            "amx-bf16", "amx-int8", "amx-fp16", "amx-complex", "amx-tf32",
            "apx-f"
        ]
    ...
    async def _process_file(self, file_path: Path) -> Iterator[InstructionRecord]:
        ...
        try:
            for xed_instruction in self.parser.parse_file(file_path):
                try:
                    instruction_record = self._convert_to_instruction_record(xed_instruction)
                    if instruction_record:
                        yield instruction_record
                except Exception as e:
                    self.log_error(f"Error converting instruction {xed_instruction.iclass}: {e}")
        except Exception as e:
            self.log_error(f"Error processing file {file_path}: {e}")
    
    def _convert_to_instruction_record(self, xed_instr: XEDInstruction) -> Optional[InstructionRecord]:
        ...
        for part in parts:
            if '=' in part and ':' in part:
                ...
                if len(parts_split) >= 2:
                    operands.append(OperandRecord(
                        name=parts_split[0],
                        type=self._normalize_operand_type(parts_split[2] if len(parts_split) > 2 else 'unknown'),
    ...
    def _parse_encoding(self, pattern_str: str) -> Optional[EncodingRecord]:
        ...
        immediate = any(imm in pattern_str for imm in [
            'UIMM8()', 'SIMM8()', 'UIMM16()', 'SIMM16()', 
            'UIMM32()', 'SIMM32()', 'SIMMz()'
        ])
    ...
    def _generate_description(self, xed_instr: XEDInstruction) -> str:
        ...
        return category_descriptions.get(xed_instr.category, f'{base_name} - {xed_instr.category} operation')
    
    def _generate_syntax(self, xed_instr: XEDInstruction, operands: List[OperandRecord]) -> str:
