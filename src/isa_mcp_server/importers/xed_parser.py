...
class XEDParser:
    ...
    def _parse_instruction_block(self, lines: List[str], start_idx: int) -> tuple[Optional[XEDInstruction], int]:
        ...
        if 'ICLASS' in instruction_data:
            instruction = XEDInstruction(
                ...
                cpl=int(instruction_data['CPL']) if instruction_data.get('CPL') else None,
                category=instruction_data.get('CATEGORY', ''),
                extension=instruction_data.get('EXTENSION', ''),
                isa_set=instruction_data.get('ISA_SET', ''),
                attributes=self._parse_attributes(instruction_data.get('ATTRIBUTES', '')),
        ...
        for part in parts:
            if '=' in part and ':' in part:
                ...
                if len(parts_split) >= 2:
                    operands.append({
                        'name': parts_split[0],
                        'type': parts_split[2] if len(parts_split) > 2 else 'unknown',
    ...
    def _parse_pattern(self, pattern_str: str) -> Dict[str, Any]:
        ...
        if any(imm in pattern_str for imm in ['UIMM8()', 'SIMM8()', 'UIMM16()', 'SIMM16()', 'UIMM32()', 'SIMM32()', 'SIMMz()']):
