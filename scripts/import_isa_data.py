...
Examples:
  ...
  %(prog)s --intel --db-path custom.db             # Use custom database path
    ...
    # ISA selection
    parser.add_argument("--all", action="store_true", help="Import all available ISAs")
    ...
    parser.add_argument(
        "--arm", action="store_true", help="Import ARM AArch64 instructions"
    )

    # Source directories
    parser.add_argument("--source-dir", type=Path, help="Source directory for ISA data")
    ...
    if not any([args.all, args.intel, args.arm]):
        parser.error(
            "Must specify at least one ISA to import (--all, --intel, or --arm)"
        )
    ...
    if args.all or args.arm:
        ...
        if not source_dir or not source_dir.exists():
            ...
            results["arm"] = await import_arm_data(
                db, source_dir, skip_metadata=args.skip_metadata
            )
