...
def populate_architecture_metadata(db: ISADatabase, xed_dir: Path) -> bool:
    ...
    try:
        ...
        for isa_name in ["x86_32", "x86_64"]:
            ...
            logging.info(f"Architecture {isa_name}: {len(registers)} registers, {len(modes)} addressing modes")
...
Examples:
  ...
  %(prog)s --verbose                                    # Enable verbose logging
    ...
    if not args.force:
        ...
        if existing_arch:
            logging.info("Architecture metadata already exists. Use --force to repopulate.")
    ...
    if success:
        ...
        for isa_name in ["x86_32", "x86_64"]:
            ...
            # Show main registers
            main_regs = [r for r in registers if r.is_main_register and r.register_class == "gpr"]
